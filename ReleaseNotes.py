...
LAST_RELEASE = '2023_03_1'
RELEASE = '2023_09_1'
LAST_RELEASE_DATE = datetime.datetime(2023, 4, 30, tzinfo=pytz.timezone('UTC'))
...
relevant = [
  i for i in ci
  if i.milestone is not None and str(i.milestone) > LAST_RELEASE and i.closed_at > LAST_RELEASE_DATE
]
bugs = []
feats = []
cleanups = []
...
for i, bug in enumerate(bugs):
  if bug.pull_request_urls is not None:
    ...
    text = 'issue'
  print("  - %s\n (github %s #%d from %s)" % (bug.title, text, bug.number, bug.user))
  seen.add(bug.number)
